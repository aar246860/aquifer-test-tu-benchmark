...
\caption{Uncertainty source matrix used to separate field response diagnostics, parameter compensation, and unresolved uncertainty sources.}
...
\renewcommand{\arraystretch}{0.95}
\caption{Parallel headline results for the Massachusetts and Lovelock Valley field cases. Entries are ordered by model: Theis confined / Hantush--Jacob leaky / lagging Darcy without leakage / lagging Darcy with leakage.}
...
\renewcommand{\arraystretch}{0.95}
\caption{Model-averaged management diagnostic for aquifer test transformation uncertainty. The model set contains the four formally fitted analytical interpretations: Theis confined, Hantush--Jacob leaky, lagging Darcy without leakage, and lagging Darcy with leakage. Rows without a strict BIC prefix use variance window BMA weights.}
...
\renewcommand{\arraystretch}{0.95}
\caption{Groundwater reporting endpoint for pumping test transformation uncertainty. The endpoint follows the evidence sequence from measured response to numerical benchmark support, field applicability, and decision consequence.}
...
\caption{Massachusetts comparison between observed and predicted log drawdown for the four main interpretation models. Each panel compares observed and predicted drawdown for one model, and colors indicate distance from the pumping well.}
\label{fig:s_scatter}
\end{figure}
...
\caption{Massachusetts distributions of the log model factor for the four main interpretation models. The vertical interval marks the 5th--95th percentile range, the marker shows the median, and numbers report pooled $SD_{\epsilon}$.}
...
\caption{Lovelock Valley comparison between observed and predicted log responses for the four main interpretation models. Each panel compares observed and predicted responses for one model, and colors indicate distance from pumping Well 10.}
...
\caption{Lovelock Valley distributions of the log model factor for the four main interpretation models. The vertical interval marks the 5th--95th percentile range, the marker shows the median, and numbers report pooled $SD_{\epsilon}$.}
