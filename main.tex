...
%% Official C&G checklist: cover letter, highlights, authorship statement,
%% single-column double-spaced manuscript, author-date references, and code availability.
...
Please consider the enclosed manuscript, "A reproducible benchmark for transformation uncertainty in aquifer-test parameter inference," for exclusive consideration as a research article in Computers \& Geosciences.
...
\caption{Numerical groundwater-flow benchmark design, implemented here with MODFLOW 6. Panel (a) shows the finite difference grid, pumping well, observation wells, and scenario-specific effective support envelope used to define the pumping-test reference. Panels (b) and (c) show representative heterogeneous and anisotropic hydraulic conductivity fields. Panel (d) shows a representative heterogeneous storage field. Panels (e) and (f) show the combined leakage, boundary, hydraulic conductivity, and storage setting used to stress the transformation from numerical drawdown response to apparent analytical parameters. The numerical fields supply the controlled drawdown responses interpreted by the four analytical solutions.}
\label{fig:mf6design}
\end{figure}
...
\centering
\scriptsize
\caption{Numerical groundwater-flow benchmark used to define the scope of field inference. MODFLOW 6 supplies the controlled response generator used here. The benchmark provides scenario-conditioned transformation model factors for the four analytical interpretations and defines the conditions under which those factors can inform field diagnostics.}
...
\caption{Analytical verification and numerical consistency checks. Panels (a) and (b) show analytical limiting behavior as a function of observation distance; the dashed line in panel (a) marks the log factor tolerance used for the limiting checks. Panels (c) and (d) show numerical consistency with analytical reference responses as grid spacing changes for homogeneous Theis and vertical leakage Hantush generators. The numerical responses are implemented with MODFLOW 6, and the warnings identify finite leakage, near well, and coarse grid limits that constrain the benchmark scope.}
...
\caption{Screened numerical benchmark model factors. Panels (a)--(c) show empirical distribution functions for transmissivity, storage, and response time model factors after excluding active boundary fits and retaining interpretations within $\Delta BIC \leq 10$. The vertical dashed line marks a factor of one. Panel (d) shows the median response log residual standard deviation by observation distance, with dashed lines marking the 90th percentile envelope. These values are scenario-conditioned model factors for the represented benchmark classes.}
\label{fig:mf6cov}
\end{figure}
...
\caption{Benchmark transfer of aquifer-test transformation uncertainty to field diagnostics. Panel (a) shows the numerical benchmark response-diagnostic space and the positions of the two field cases. Panel (b) compares cross-validated predictions with observed 90th percentile absolute log model factors for transmissivity, storage, and response time. Panel (c) reports the main diagnostic variables used by the regression. Panel (d) gives the benchmark-transferred, 90th-percentile-equivalent model-factor COV values for the two field cases.}
...
\caption{Paired field data spatial support for the two real cases. (a) Massachusetts fractured bedrock retained observation wells. (b) Lovelock Valley MWP2 retained observation wells relative to pumping Well 10. Both panels use coordinates centered on the pumping well and normalized by the maximum retained radial support within the case, so panel shape shows observation geometry rather than absolute field scale. Circles mark retained observation wells, stars mark pumping wells, and colors retain the actual radial distance within each case.}
\label{fig:layout}
\end{figure}
...
\caption{Massachusetts observed and simulated drawdown curves for the 12 observation wells under the four main interpretation models. Points are measured drawdown; lines show Theis confined, Hantush--Jacob leaky, lagging Darcy without leakage, and lagging Darcy with leakage predictions.}
\label{fig:drawdown}
\end{figure}
...
\caption{Massachusetts summary for the interpretation models. Panels report pooled log model factor dispersion, BIC preference counts across the four main interpretation models, lognormal parameter COV for fitted transmissivity and storage, and competing BIC support relative to lagging Darcy without leakage.}
...
\caption{Lovelock Valley MWP2 observed drawdown/recovery and fitted response curves under the four main interpretation models. Panels show the seven selected observation wells ordered by distance from pumping Well 10. Points are measured responses, lines show the four model predictions, the shaded interval marks the pumping period, and the dotted line marks shutoff.}
...
\caption{Lovelock Valley summary for the interpretation models. Panels report pooled log model factor dispersion, BIC preference counts across the four main interpretation models, lognormal parameter COV for fitted transmissivity and storage, and competing BIC support relative to lagging Darcy without leakage.}
...
\caption{Validation synthesis based on the Massachusetts fractured bedrock case and the Lovelock Valley basin fill case. Panels compare pooled log model factor dispersion, BIC preference counts by well, support for models that include leakage, transmissivity COV, storage COV, and the scope of inference separating response support, leakage support, parameter interpretation, and spatial transfer availability.}
...
\caption{Field application of scenario-conditioned transformation model factors after the field evidence chains have been established. Panel (a) maps the two field cases to numerical benchmark classes and applicability levels. Panel (b) shows the matched transmissivity and storage model factor COV values used as field sensitivity brackets or conservative priors. Panel (c) reports field BIC support for the four analytical interpretations. Panel (d) compares inherited 95th percentile capacity and response time factors from the model averaged decision diagnostic. The figure identifies where benchmark factors are carried into the field interpretation.}
...
\caption{Model-averaged propagation of aquifer test interpretation uncertainty to management-relevant hydraulic quantities. (a) Mean variance window BMA weights for the four analytical interpretations. (b) Median and 95th percentile hydraulic diffusivity spread, expressed as $D_{95}/D_{05}$ with $D=T/S$. (c) Median robust hydraulic control capacity factor under strict BIC and variance window weighting. (d) Mean variance window BMA probability that a selected interpretation underestimates hydraulic control capacity or misses an earlier hydraulic response.}
